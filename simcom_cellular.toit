...
class UdpSocket extends Socket_ implements udp.Socket:
  ...
  constructor cellular/SimcomCellular id/int port/int:
    super cellular id
...
/**
Base driver for Simcom Cellular devices, communicating over CAT-NB1 and/or CAT-M1.
*/
abstract class SimcomCellular extends CellularBase implements Gnss:
  ...
  configure apn --bands=null --rats=null:
    at_.do: | session/at.Session |
      ...
      while true:
        ...
        // session.action "+CGATT=1"
        // session.action "+CSQ "


        // Set bearer params
        session.set "+SAPBR" [3,1,"Contype","GPRS"]
        session.set "+SAPBR" [3,1,"APN","giffgaff.com"]
        session.set "+SAPBR" [3,1,"USER","gg"]
        session.set "+SAPBR" [3,1,"PWD","p"]

        // Check if connected
        while not session.action "+CGATT?":
          sleep --ms=15000

        session.set "+SAPBR" [1,1]

        // Query connection details
        session.set "+SAPBR" [2,1]
